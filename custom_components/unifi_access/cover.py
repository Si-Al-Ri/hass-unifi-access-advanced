...
class UnifiAccessCoverEntity(UnifiAccessDoorEntity, CoverEntity):
    ...
    A UA Hub Gate exposes a momentary unlock trigger, not distinct open/close
    commands, so travel is inferred from a configurable open/close timer plus
    the door position sensor (DPS). If the timer expires but the DPS has not
    reached the expected state, the door is flagged as obstructed.
    """
