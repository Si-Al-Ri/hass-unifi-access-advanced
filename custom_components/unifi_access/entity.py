...
def manage_door_entities(
    ...
    A door's ``entity_type`` (lock / garage / gate) can change at runtime via
    the EntityTypeSelect, so each platform declares a ``should_include``
    predicate and an entity factory. This helper adds entities for newly
    matching doors and removes them (from the entity registry) for doors that
    no longer match, keeping the three platforms mutually exclusive per door.
    """
...
class UnifiAccessReaderEntity(
    CoordinatorEntity[UnifiAccessCoordinator[dict[str, DoorState]]]
):
    """Base entity for an access reader (intercom / reader) device.

    Used for access-method entities (face unlock, NFC, PIN, ...) which belong
    to the reader device, not the door hub. Bound to the door coordinator so
    reader-settings refreshes (via _notify_doors_updated) push state updates.
    """
    ...
    def __init__(
        ...
        """Initialize the reader-bound entity."""
        super().__init__(coordinator, context=reader.device_id)
        self._hub = hub
        self.reader = reader

    @property
    def device_info(self) -> DeviceInfo:
        """Return device info for the access reader."""
        return DeviceInfo(
            identifiers={(DOMAIN, self.reader.device_id)},
