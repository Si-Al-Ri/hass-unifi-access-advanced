...
# Standard "ring" event type for DOORBELL-class event entities.
# ``DoorbellEventType`` is available from HA 2026.4; older cores use the
# literal value "ring".
try:
    from homeassistant.components.event import DoorbellEventType

    RING_EVENT: str = DoorbellEventType.RING
except ImportError:
    RING_EVENT = "ring"
...
class DoorbellPressedEventEntity(_UnifiAccessEventEntity):
    ...
    A doorbell press fires two state updates back to back: the standard
    ``ring`` event, then the ``unifi_access_doorbell_start`` event. Publishing
    the start event last leaves it as the resting event type until the release.
    Release fires ``unifi_access_doorbell_stop``.
    """
    ...
    def _async_handle_event(self, event: str, event_attributes: dict[str, str]) -> None:
        """Fire the ``ring`` event before the start event on a press."""
        event_type = event_attributes.get("type", event)
        if event_type == DOORBELL_START_EVENT:
            self._trigger_event(RING_EVENT, event_attributes)
            self.async_write_ha_state()
        self._trigger_event(event_type, event_attributes)
        self.async_write_ha_state()
